...
export default class Month extends React.Component {
  ...
  onMonthClick = (e, m, year) => {
    this.handleDayClick(
      utils.getStartOfMonth(
        utils.setMonth(utils.setYear(this.props.day, year), m)
      ),
      e
    );
  };
  ...
  renderMonths = () => {
    let months = [];
    const { monthsList, year } = this.props;
    if (monthsList && monthsList.length > 0) {
      while (monthsList.length) months.push(monthsList.splice(0, 3));
    } else {
      months = [
        [0, 1, 2],
        [3, 4, 5],
